/**
 * POST /a/verify/resend — re-issue the OTP for the current session.
 *
 * Throttle (app-level, mirroring presales — /a/* page routes are not
 ...
 *
 * On exhaustion the user is sent back to /a/verify with an error. Resend
...
import { canResendOtp, issueGuestOtp } from "@/lib/affirm/external/otp";
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  if (!verifySessionNonce(ctx.session.id, csrf)) {
    return redirectTo(req, "/a/verify?error=resend_rate_limited");
  }

  const now = new Date();

  // Session-scoped resend cap.
  const resendCount = await prisma.affirmEvent.count({
    where: {
  ...
  if (resendCount >= MAX_RESENDS_PER_SESSION) {
    return redirectTo(req, "/a/verify?error=resend_exhausted");
  }
  ...
  await issueGuestOtp({ grantId: ctx.grant.id, now, kind: "resend", sessionId: ctx.session.id });
  return redirectTo(req, "/a/verify");
}
