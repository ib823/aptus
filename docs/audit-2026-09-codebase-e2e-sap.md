...
**Withdrawn from this table (follow-up PR).** The first draft of this report listed `POST /api/sap/tdd/write` and `hub-content/write-test` as High because they resolve deployment tenants only (`getSapTenant`) and not stored connections, "the read-route defect left in place". That reading was wrong, and the follow-up PR that set out to wire them to `resolveReadTenant` stopped when it found why: `tests/unit/studio/tenant-registries.test.ts` exempts exactly those two routes with a written rationale — the write ring is gated by `{PREFIX}_WRITE_ENABLED`, `{PREFIX}_WRITE_SECRET` and `{PREFIX}_WRITE_TEST_ENTITYSETS`, all deployment settings describing the deployment's own test ring; the write panel's tenant list comes from `/api/sap/tdd/catalog`, which lists deployment tenants only, so no screen ever offers a connection key there; and a stored connection is written through the northbound broker (`src/lib/northbound/write.ts`), which binds the write to a declared environment and a grant. Pointing the env-secret-protected ring at a customer connection would have been a second, ungoverned write path into a customer system. What was real is the refusal text: an unknown key got "Valid tenant, service, entity, and object payload are required", four things named and none of them the problem. The follow-up PR replaces that with `deploymentOnlyTenantMessage`, which names the key, the registry writes use and where a connection *is* written, and extends the scan test to pin the scoping in both directions (the routes may not consult the connection registry, and must explain the refusal).
...
| M-6 | Medium → **partly fixed (follow-up PR)** | E2E coverage is thinner than the scripts suggest: CI runs four smoke specs plus the discovery specs; the admin/PO/IT/exec/journeys/edge-cases/responsive projects never run in CI; journeys self-skip without seeds and nothing invokes `tests/seed/*`. `tests/e2e/discovery-seam.spec.ts` matched no project and had never been collected anywhere | `.github/workflows/ci.yml`, `playwright.config.ts` | Done: renamed to `discovery-seam.consultant.spec.ts` and added to `test:e2e:discovery-session`, so CI runs it. Running it for the first time found three things the spec had wrong, all fixed in the spec: its engagement had no `createdById`, so the facilitate page 404'd under the ownership guard; its "first" process (`31G`) is Concept-to-Market, outside the session's Source-to-Pay scope, so the guest page redirected home; and it pressed `p` before hydration, so Present mode never engaged and the follower was never mounted. The product itself held up: all seven tests pass, including both privacy assertions. Its first CI run then exposed a fourth thing, in CI rather than the spec: the e2e job serves the app with `pnpm start` (production), and no guest route (`/a/*`, `/c/*`, `/d/*`) had ever been exercised there, so nobody had noticed the job env lacked `PRESALES_CSRF_SECRET` — which the guest CSRF code refuses to fall back from in production. Every landing 500'd. The workflow now sets a CI-only value, and the spec asserts the landing's status so that class of failure names itself instead of timing out. Still open: a seeded e2e job with `REQUIRE_SEED=1` for the journeys; `discovery-capture` and `discovery-workbench` consultant specs carry the same "never executed" note and are not in CI. |
...
| M-9 | Medium → **fixed (follow-up PR)** | Docs claimed TOTP MFA that does not exist (passkey-only shipped); `TotpSetupForm`/`TotpVerifyForm` had no importers; `TOTP_ENCRYPTION_KEY`/`TOTP_ISSUER` were set in CI and read nowhere; a Security Settings tour step targeted a `[data-tour="mfa-section"]` that no page renders; `CONTRIBUTING.md` said the Playwright image was not digest-pinned and visual regression non-blocking while `ci.yml` pins by digest with no `continue-on-error` | `docs/BUILD-PHASES-STATUS.md`, `docs/HANDOFF.md`, `docs/SCENARIOS-E2E.md`, `docs/ACTOR-PROCESS-E2E.md`, `scripts/generate-user-manual.js`, `src/lib/tour/tours.ts`, `CONTRIBUTING.md` | Done: docs and the tour corrected, the two components, the CI env and the page-object `fillTotp` helpers deleted, the fictional "MFA UX Validation" block dropped from `tests/unit/polish.test.ts`. Left as-is, deliberately: `tests/security/authentication.test.ts` T-SEC-008 still exercises an inline stand-in `verifyTotp`; `MfaMethod` in `src/types/assessment.ts` still admits `"totp"`; the `fit-portal-transfer/specs/*` and `specs/v2/*` design documents describe TOTP as designed, and are history rather than claims. |
...
4. ~~Fix the two remaining SAP write-route tenant resolutions~~ — withdrawn; see the note under §4.5. The refusal message is fixed in the follow-up PR, the scoping is by design.
